...
def ohlc_missing_data():
    ...
    return pd.DataFrame({
        'Open': [100.0, np.nan, 99.8, 102.1, np.nan, 103.2, 101.7, np.nan, 102.3, 105.1],
        'High': [102.3, 103.0, np.nan, 103.5, 102.0, np.nan, 103.1, 106.2, np.nan, 106.5],
        'Low': [99.5, np.nan, 98.9, np.nan, 100.1, 102.5, np.nan, 103.8, 101.5, np.nan],
        'Close': [101.2, 102.5, np.nan, 102.8, 101.5, np.nan, 102.4, 105.7, np.nan, 105.8]
    })
...
def test_edge_missing(ohlc_missing_data):
    ...
    assert np.isfinite(estimate) or np.isnan(estimate), f"Should handle missing data gracefully, got {estimate}"
