...
class DataFetcher:
    ...
    async def get_btc_1m_websocket(self, exchange_name: str = "binance", duration_seconds: int = 60):
        ...
        try:
            ...
            logger.info(f"🚀 Starting BTC real-time data stream for {duration_seconds}s using {exchange_name}")
            ...
            for i in range(duration_seconds):
                try:
                    ...
                    if bid and ask and bid > 0 and ask > 0:
                        ...
                        # If real spread is 0 or very small, use estimated spread instead
                        if spread_value < price * 0.000001:  # Less than 0.1 bps
                            base_spread_bps = 2.0  # 2 basis points realistic spread
                        ...
                        volatility_factor = 1 + np.random.uniform(-0.5, 1.0)  # Add some randomness
                        spread_bps = base_spread_bps * volatility_factor
                        spread_value = price * (spread_bps / 10000)
                        spread_source = "estimated"
                    ...
                    data_points.append({
                        ...
                        'symbol': 'BTCUSDT',
                        'price': price,
                        'open': float(ticker['open']) if ticker.get('open') else price,
                        'high': float(ticker['high']) if ticker.get('high') else price,
                    ...
                    if i % 5 == 0:  # Log every 5 seconds
                        logger.info(f"📊 Real BTC: ${price:.2f}, spread: {spread_bps:.2f}bps ({spread_source})")
            ...
            if data_points:
                ...
                logger.info(f"✅ Collected {len(data_points)} real BTC data points from {exchange_name}")
                return df
            else:
    ...
    def _generate_synthetic_btc_stream(self, duration_seconds: int):
        ...
        logger.info(f"🧪 Generating synthetic BTC data with realistic price movements for {duration_seconds}s")
        ...
        for i in range(duration_seconds):
            ...
            if i % 10 == 0:  # Log every 10 seconds
                logger.info(f"🎲 Synthetic BTC: ${price:.2f} (change: {((price/base_price-1)*100):+.3f}%), spread: {spread_bps:.2f}bps")
            
            base_price = price  # Use current price as base for next
        
        logger.info(f"Generated {len(data_points)} synthetic BTC data points with price range ${min(d['price'] for d in data_points):.2f}-${max(d['price'] for d in data_points):.2f}")
